...
class BooleanBrick extends AbstractBrick
{
    public function test(LexerBrickRequest $request)
    {
        ...
        if (substr($request->source, $request->pos, 4) === 'true') {
            $c = substr($request->source, $request->pos + 4, 1);

            if ($c === false || 1 === preg_match($this->delimiterRegex, $c)) {
                $response->tokenName = 'boolean';
                $response->newPos = $request->pos + 4;
            }
        } elseif (substr($request->source, $request->pos, 5) === 'false') {
            $c = substr($request->source, $request->pos + 5, 1);

            if ($c === false || 1 === preg_match($this->delimiterRegex, $c)) {
                $response->tokenName = 'boolean';
                $response->newPos = $request->pos + 5;
            }
        }
